...
bool FUberLitRenderPass::Initialize()
{
    return true;
}

bool FUberLitRenderPass::Begin(const FRenderPassContext* Context)
{
...
bool FUberLitRenderPass::DrawCommand(const FRenderPassContext* Context)
{
    ...
    for (const FRenderCommand& Cmd : Commands)
    {
        ...
        if (Cmd.Material)
        {
            UMaterial* Material = dynamic_cast<UMaterial*>(Cmd.Material);

			if (Material)
			{
                // Material->ShaderKey.ShadingModel = EShadingModel::LIGHTING_MODEL_GOURAUD;
                // Material->ShaderKey.ShadingModel = EShadingModel::LIGHTING_MODEL_LAMBERT;
                 Material->ShaderKey.ShadingModel = EShadingModel::LIGHTING_MODEL_PHONG;
			}
...
bool FUberLitRenderPass::End(const FRenderPassContext* Context)
{
    return true;
}

bool FUberLitRenderPass::Release()
{
    return true;
}
